...
void kriza(){
  ...
  faza = 1;
  prikaz.acc = prikaz.acc + Bod(100, 0);
  prikaz.ciel = Bod();
  hybeSa = true;
}
...
bool niecoVCeste(){
  ...
  int zrazitelne[] = {STRELA, ASTEROID, PLANETA, BOSS};
  FOR(i, 4){
    FOR(j, stav.obj[zrazitelne[i]].size()){
      if(prekazkaVCeste(stav.obj[zrazitelne[i]][j])) return 0;
    }
  }
...
void zistiTah() {
  ...
  if(fixID != -1 && fixID < stav.obj[ZLATO].size()){
    ...
    FOR(i, stav.obj[ZLATO].size()){
      if(priamaVzd(stav.obj[ZLATO][i].rychlost, Bod()) <= EPS){
