...
Bod najdiCiel(){
  ...
  while(tried<10){
    ...
    tried++;
    double time = alpha(stav.hraci[id].obj, 0, 10);
    Bod d = Bod(stav.hraci[id].obj.rychlost.x * (time+COOLDOWN), stav.hraci[id].obj.rychlost.x * (time+COOLDOWN));
    Bod pozi = d + stav.hraci[id].obj.pozicia;
    if(pozi.dist() < minDist){
      minDist = pozi.dist();
      poz = pozi;
    }
  }
...
void kriza(){
  ...
  prikaz.ciel = Bod();
  double dxx = -stav.hraci[0].obj.rychlost.x*3, dyy = -stav.hraci[0].obj.rychlost.y*2;
  faza = 1;
...
bool niecoVCeste(){
  ...
  FOR(i, 3){
    FOR(j, stav.obj[zrazitelne[i]].size()){
      if(prekazkaVCeste(stav.obj[zrazitelne[i]][j]) < 0.3 && prekazkaVCeste(stav.obj[zrazitelne[i]][j])!=-1){
	//cerr << i << " " << j << endl;
	return 1;
      }
    }
  ...
  Bod pos = stav.hraci[0].obj.rychlost * 0.3 + stav.hraci[0].obj.pozicia;
  ...
  FOR(i, stav.hraci.size()){
    if(i!=0){
      if(prekazkaVCeste(stav.hraci[i].obj) < 0.3 && prekazkaVCeste(stav.hraci[i].obj)!=-1) return 1;
    }
  }
...
void zastav(){
  prikaz.acc = kontroler(Bod(), stav.hraci[0].obj.rychlost, Bod(10, 10));
}
...
void zistiTah() {
  ...
  if(mode == 'B'){
    ...
    cerr << prikaz.acc.x << prikaz.acc.y << endl;
