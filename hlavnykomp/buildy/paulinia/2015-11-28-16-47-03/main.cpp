...
void kriza(){
  ...
  prikaz.ciel = Bod();
  double dxx = -stav.hraci[0].obj.rychlost.x*2, dyy = stav.hraci[0].obj.rychlost.y*2;
  faza = 1;
...
int main() {
  ...
  while (cin.good()) {
    ...
    if(zz) zistiTah();
      ...
      prikaz.acc = kontroler(stav.hraci[0].obj.pozicia, Bod(0, 0), stav.hraci[0].obj.pozicia + Bod(rand()%10, rand()%10));
      prikaz.ciel = Bod();
    }
    zz++;
