...
void zistiTah() {
   ...
   if (!problemstena(ja))
   {
       ...
       if(citac%4200==0) nasoby *=-1;
       
        if(citac%50==0)
        {
           
            if(celkova_rychlost<320)
            {
                akcel.x+=rand()%150*nasobx;
                akcel.y+=rand()%150*nasoby;
            }
        }
   }
    //uhybaj sa objektom
   ...
   // for(int i = 0; i<stav.obj[PLANETA].size();i++){na_vyhnutie.push_back(stav.obj[PLANETA][i]);}
    ...
    for(int i = 0; i<na_vyhnutie.size(); i++){
        if((ja.obj.pozicia-na_vyhnutie[i].pozicia).dist()< (na_vyhnutie[i].polomer+ja.obj.polomer)*1.50){
                ...
                if(ja.obj.rychlost.x>=0) akcel.x -= 100;
                else akcel.x += 100;
                if(ja.obj.rychlost.y>=0) akcel.y -= 100;
                else akcel.y += 100;
    ...
    for(int i =1; i<stav.hraci.size(); i++){
        if(stav.hraci[i].zije() == true && (stav.hraci[i].obj.pozicia-ja.obj.pozicia).dist()<_min_dist){_min=i;}
        if(stav.hraci[_min].zije() == true) _min_dist=(stav.hraci[_min].obj.pozicia-ja.obj.pozicia).dist();
    }
    if(_min_dist<250){
        strela = stav.hraci[_min].obj.pozicia-ja.obj.pozicia;
    }else strela = Bod();
    
    //akcel.x = long long(akcel.x) % 201;
    //akcel.y = long long (akcel.y) % 201;
