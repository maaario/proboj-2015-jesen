...
Bod sprint(Bod k){
  return (k - stav.hraci[0].obj.pozicia)*0.6;
...
Bod zlato()
{
  ...
  Bod D=stav.obj[4][x].pozicia-stav.hraci[0].obj.pozicia;
  Bod E=D-stav.hraci[0].obj.rychlost;
  if(D.dist()<100)return E*3;
  else 
  {
    if(D.dist()<50)return E*5;
    return E;
  }
}
int setrage(){
...
void zistiTah() {
  ...
  prikaz.acc=odsteny*5+odobj+zzz*2;
  if(stav.hraci[0].obj.rychlost.dist()>200)
    prikaz.acc=prikaz.acc+(stav.hraci[0].obj.rychlost*(-1));
}
