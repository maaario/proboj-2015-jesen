...
bool mimomapy(Bod A)
{
  if(A.x<0 || A.y<0 || A.x>=mapa.w || A.y>=mapa.h)return false;
  return true;
}
Bod bachastena()
{
  ...
  if(mimomapy(Z))return X+Y;
  else return sprint(stav.hraci[korist].obj.pozicia);
  // if(mapa.w/20>stav.hraci[0].obj.pozicia.x || mapa.h/20>stav.hraci[0].obj.pozicia.y || 19*mapa.w/20<stav.hraci[0].obj.pozicia.x || 19*mapa.h/20<stav.hraci[0].obj.pozicia.y){
  // else 
...
Bod baena(){
  ...
  if ((np+kv).x>2*r1 && (np+kv).y>2*r1 && (np+kv).x+2*r1<mapa.w && (np+kv).y+2*r1<mapa.h)
    return stav.hraci[0].obj.pozicia+kv*20;
  return stav.hraci[0].obj.pozicia-kv*20;
}
Bod pozorobj()
{
...
void zistiTah() {
  ...
  korist=najblizsi();
  prikaz.ciel=stav.hraci[korist].obj.pozicia-stav.hraci[0].obj.pozicia;;
