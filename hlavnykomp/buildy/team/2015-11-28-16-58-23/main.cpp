...
int najblizssi_bod_v_poli(vector<FyzickyObjekt> pole) {
    double maximalna_vzdialenost = 750;
    maximalna_vzdialenost *= maximalna_vzdialenost;
    double best[2] = {10000000, 0};
...
Bod zmen_poziciu(Bod pozicia) {
    ...
    */
    ...
    Bod rychlost = Bod((pozicia.x - ja.pozicia.x) / d_t,
                       (pozicia.y - ja.pozicia.y) / d_t);
    double predelit = abs(max(rychlost.x, rychlost.y) / maximalna_rychlost);
    rychlost.x /= predelit;
    rychlost.y /= predelit;
    Bod akceleracia = Bod(rychlost.x / d_t, rychlost.y / d_t);
...
void zistiTah() {
    ...
    if (zamiereny_hrac) {
        if ((ja.pozicia - stav.hraci[zamiereny_hrac].obj.pozicia).dist2() > 400 * 400)
