...
Bod kontroler(Bod pozicia, Bod rychlost, Bod cielovaPozicia) {
	...
	for (unsigned int i = 1; i < stav.hraci.size(); i++) {
		if ((stav.hraci[i].obj.pozicia-pozicia).dist() < 50) {
	...
	for (unsigned int i = 0; i < stav.obj[STRELA].size(); i++) {
		if ((stav.obj[STRELA][i].pozicia-pozicia).dist() < 30) {
			Bod d = (pozicia-stav.obj[STRELA][i].pozicia);
			cielovaPozicia = pozicia + Bod(d.y,d.x)*50;
		}
	}
...
void zistiTah() {
	...
	*/
	...
	else prikaz.ciel = stav.cas > 1 ? Bod() : Bod(100,100);
