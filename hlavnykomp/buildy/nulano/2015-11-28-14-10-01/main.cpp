...
Bod kontroler(Bod pozicia, Bod rychlost, Bod cielovaPozicia) {
	...
	if (pozicia.x < 40) pozicia.x = 40;
	if (pozicia.y < 40) pozicia.y = 40;
	if (pozicia.x > mapa.w-40) pozicia.x = mapa.w-40;
	if (pozicia.y > mapa.h-40) pozicia.y = mapa.h-40;
...
void zistiTah() {
	...
	if (stav.obj[BOSS].size()>0) {
		cerr << "Evade the boss instead" << endl;
		bkam = (stav.obj[BOSS][0].pozicia - stav.hraci[0].obj.pozicia)*1000;
	}
