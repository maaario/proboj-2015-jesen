...
Bod kontroler(Bod pozicia, Bod rychlost, Bod cielovaPozicia) {
	...
	cerr << "Ciel pred: [" << pozicia.x << ", " << pozicia.y << "]" << endl;
	if (pozicia.x < 100) pozicia.x = 120;
	if (pozicia.y < 100) pozicia.y = 120;
	if (pozicia.x > mapa.w-100) pozicia.x = mapa.w-120;
	if (pozicia.y > mapa.h-100) pozicia.y = mapa.h-120;
	cerr << "Ciel pred: [" << pozicia.x << ", " << pozicia.y << "]" << endl;
...
void zistiTah() {
	...
	if (stav.obj[BOSS].size()>0 && (stav.hraci[0].obj.pozicia-stav.obj[BOSS][0].pozicia).dist() < 200) {
