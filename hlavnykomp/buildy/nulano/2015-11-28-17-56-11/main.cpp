...
Bod kontroler(Bod pozicia, Bod rychlost, Bod cielovaPozicia) {
	...
	for (unsigned int i = 0; i < stav.obj[ASTEROID].size(); i++) {
		if ((stav.obj[ASTEROID][i].pozicia-pozicia).dist() < stav.obj[ASTEROID][i].polomer*4) {
	...
	for (unsigned int i = 1; i < stav.hraci.size(); i++) {
		if ((stav.hraci[i].obj.pozicia-pozicia).dist() < 100) {
			cielovaPozicia = pozicia + (pozicia-stav.hraci[i].obj.pozicia);
		}
	}
	for (unsigned int i = 0; i < stav.obj[STRELA].size(); i++) {
		if (stav.obj[STRELA][i].owner == stav.hraci[0].obj.owner) continue;
		if ((stav.obj[STRELA][i].pozicia-pozicia).dist() < 400) {
			Bod d = (pozicia-stav.obj[STRELA][i].pozicia);
			cielovaPozicia = pozicia - Bod(d.y,d.x)*5000;
		}
	}
	...
	if (cielovaPozicia.x < 120) cielovaPozicia.x = 150;
	if (cielovaPozicia.y < 120) cielovaPozicia.y = 150;
	if (cielovaPozicia.x > mapa.w-120) cielovaPozicia.x = mapa.w-150;
	if (cielovaPozicia.y > mapa.h-120) cielovaPozicia.y = mapa.h-150;
	cerr << "Ciel po: [" << cielovaPozicia.x << ", " << cielovaPozicia.y << "]" << endl;
...
void zistiTah() {
	...
	for (unsigned int i = 0; i < stav.obj[ZLATO].size(); i++) {
		if (stav.obj[ZLATO][i].pozicia.x < 150 || stav.obj[ZLATO][i].pozicia.x > mapa.w-150 || 
			stav.obj[ZLATO][i].pozicia.y < 150 || stav.obj[ZLATO][i].pozicia.y > mapa.h-150
		) continue;
	...
	Bod bkam = stav.cas < 1 ? Bod(400,400) : stav.hraci[0].obj.pozicia;
	if (kam >= 0) {
