...
void zistiTah() {
	...
	unsigned int kam = -1; double mindist = 789456;
	for (unsigned int i = 0; i < stav.obj[ZLATO].size(); i++) {
		if (stav.obj[ZLATO][i].rychlost.dist() > 50) continue;
		if ((stav.hraci[i].obj.pozicia - stav.obj[ZLATO][i].pozicia).dist() < mindist) {
			kam = i; mindist = (stav.hraci[i].obj.pozicia - stav.obj[ZLATO][i].pozicia).dist();
		}
	}
	Bod bkam = stav.hraci[0].obj.pozicia;
	if (kam >= 0) bkam = stav.obj[ZLATO][kam].pozicia;
