...
Vec zratajxclenku(Vec pos, Vec rychlost)
{
    double mi=0,ma=6;
    while((ma-mi)>0.001)
    {
        double mid=(ma+mi)/2;
        if(abs(pos+mid*rychlost)/STRELA_RYCHLOST>mid)
        {
            mi=mid;
        }
        else
...
void zistiTah() {
  ...
  DoStuff(konRych, square, ZLATO,    150000000 / stav.obj[ZLATO].size() + 1, mojavec);
  Vec tempboss;
  DoStuff(tempboss, linear, BOSS, -1200000, mojavec);
  tempboss*=Vec(0,1);
  konRych+=tempboss;
  DoStuff(konRych, square, ASTEROID,-6000000, mojavec);
  DoStuff(konRych, square, PLANETA, -6000000, mojavec);
  Vec temp;
  DoStuff(temp, [](double d){return d;}, STRELA, -10000, mojavec);
  ...
  double kmagiaodstien=16000000000;
  vx=max(min(vx,kmagiaodstien*(max(0.0,mapa.w-25*LOD_POLOMER-mojavec.real()))-15),-kmagiaodstien*(max(0.0,mojavec.real()-25*LOD_POLOMER))+15);
  vy=max(min(vy,kmagiaodstien*(max(0.0,mapa.h-25*LOD_POLOMER-mojavec.imag()))-15),-kmagiaodstien*(max(0.0,mojavec.imag()-25*LOD_POLOMER))+15);
  konRych=Vec(vx,vy);
  ...
  for(Hrac h:stav.hraci)
  {
      ...
      if((abs(vecovat(h.obj.pozicia) - mojavec)+ h.obj.zivoty) < minscore )
      {
          ...
          prikaz.ciel=odvecovat(zratajxclenku(vecovat(h.obj.pozicia)-mojavec,vecovat(h.obj.rychlost)-vecovat(ja.obj.rychlost)));
      }
  }
  bool debugstrely=true;
  if(mojavec.real()<(20*LOD_POLOMER)) {if(debugstrely) prikaz.ciel=Bod(0,0); prikaz.acc=prikaz.acc+Bod(1000000,0);}
  if(mojavec.imag()<(20*LOD_POLOMER)) {if(debugstrely) prikaz.ciel=Bod(0,0); prikaz.acc=prikaz.acc+Bod(0,1000000);}

  if(mojavec.real()>(mapa.w-20*LOD_POLOMER)) {if(debugstrely) prikaz.ciel=Bod(0,0); prikaz.acc=prikaz.acc-Bod(1000000,0);}
  if(mojavec.imag()>(mapa.h-20*LOD_POLOMER)) {if(debugstrely) prikaz.ciel=Bod(0,0); prikaz.acc=prikaz.acc-Bod(0,1000000);}
}
