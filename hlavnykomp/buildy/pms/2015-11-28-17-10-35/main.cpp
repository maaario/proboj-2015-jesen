...
void zistiTah() {
  ...
  double kmagiaodstien=80000000000;
  vx=max(min(vx,kmagiaodstien*(max(0.0,mapa.w-14*LOD_POLOMER-mojavec.real()))-15),-kmagiaodstien*(max(0.0,mojavec.real()-14*LOD_POLOMER))+15);
  vy=max(min(vy,kmagiaodstien*(max(0.0,mapa.h-14*LOD_POLOMER-mojavec.imag()))-15),-kmagiaodstien*(max(0.0,mojavec.imag()-14*LOD_POLOMER))+15);
  konRych=Vec(vx,vy);
  ...
  if(mojavec.real()<(10*LOD_POLOMER)) {if(debugstrely) prikaz.ciel=Bod(0,0); prikaz.acc=prikaz.acc+Bod(1000000,0);}
  if(mojavec.imag()<(10*LOD_POLOMER)) {if(debugstrely) prikaz.ciel=Bod(0,0); prikaz.acc=prikaz.acc+Bod(0,1000000);}

  if(mojavec.real()>(mapa.w-10*LOD_POLOMER)) {if(debugstrely) prikaz.ciel=Bod(0,0); prikaz.acc=prikaz.acc-Bod(1000000,0);}
  if(mojavec.imag()>(mapa.h-10*LOD_POLOMER)) {if(debugstrely) prikaz.ciel=Bod(0,0); prikaz.acc=prikaz.acc-Bod(0,1000000);}
}
